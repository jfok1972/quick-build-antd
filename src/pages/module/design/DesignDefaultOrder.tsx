...
const saveDefaultOrderScheme = (details: any[], objectRecord: any) => {
  saveDefaultOrderSchemeDetails({
    dataObjectId: objectRecord.objectid,
    schemeDefine: JSON.stringify(getChildNodesArray(details[0])),
  }).then((response) => {
    if (response.success) {
      message.success(`模块『${objectRecord.title}』的默认排序方案已保存。`);
    } else {
      Modal.error({
        title: `排序方案保存失败！`,
        width: 500,
...
export const DesignDefaultOrder: React.FC<DesignDefaultOrderProps> = ({ objectRecord }) => {
  const moduleName = objectRecord.objectid;
      ...
      <Row gutter={16} style={{ height: 'calc(100% )' }}>
        ...
        <Col lg={7} md={12} sm={12} xs={24}>
          <Card
            ...
            extra={
              <Space>
                <a href="#" onClick={() => saveDefaultOrderScheme(details, objectRecord)}>
                  保存
                </a>
              </Space>
            }
